...
**BEFORE ANY WORK BEGINS**, read the entire current manuscript (`87.0IRH.md`) from start to finish using the `view` tool. This is the single source of truth for the IHM-HRIIP theoretical framework. Without full comprehension of its contents — all derivations, confidence scores, open problems, and cross-references — you cannot produce contextually correct work.

**How to comply:** At the start of every session, execute:
```
view(path="/home/runner/work/AgentsOfAcademia/AgentsOfAcademia/87.0IRH.md")
```
...
> "Before starting, read the entire manuscript 87.0IRH.md for full theory context."
...
3. **`87.0IRH.md`** (current manuscript) — Integrate finalized theoretical content
