...
def construct_gamma_from_root_vectors():
    ...
    With the Lorentzian Clifford algebra used here,
        {γ^μ, γ^ν} = 2η^{μν} I₄,   η = diag(+1, -1, -1, -1),
    the corresponding quadratic relation is
        Γ_R² = (δ_R)_μ (δ_R)_ν η^{μν} I₄.

    Note that this is a Minkowski contraction, not the Euclidean D₄ root
    norm. Although every D₄ root has Euclidean length |δ_R|² = 2, the
    Minkowski value η(δ_R, δ_R) depends on the root and can be +2, 0, or -2.

    This means the D₄ root vectors define slashed operators inside the same
    rank-4 Clifford algebra generated by the Dirac gamma matrices.
    """
